<?= $this->include('include/admin_header') ?>
    ...
    <div class="container-fluid px-4">
        <h1 class="mt-4">Update Event with Tickets</h1>
        <ol class="breadcrumb mb-4">
            <li class="breadcrumb-item"><a href="index.html">Dashboard</a></li>
            <li class="breadcrumb-item active">Update Event with Tickets</li>
        </ol>
        <div class="card mb-4">
            <div class="card-body">
                <form action="<?= site_url('admin/events/update/'.$eventID) ?>" method="post" enctype="multipart/form-data">
                    <!-- Hidden input field to store EventID -->
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="eventLocation" name="eventLocation" value="<?= $eventLocation ?>" required>
                    </div>
                    <hr>
                    <h2>Ticket Types</h2>
                    <div id="ticketTypes">
                        ...
                        <div class="mb-3">
                            <label for="ticketType<?= $index + 1 ?>" class="form-label">Ticket Type</label>
                            <input type="text" class="form-control" id="ticketType<?= $index + 1 ?>" name="ticketTypes[]" value="<?= $ticketType['TicketType'] ?>" required>
                        </div>
                        <div class="mb-3">
                            <label for="price<?= $index + 1 ?>" class="form-label">Price</label>
                            <input type="number" class="form-control" id="price<?= $index + 1 ?>" name="prices[]" value="<?= $ticketType['Price'] ?>" required>
                        </div>
                        <div class="mb-3">
                            <label for="quantity<?= $index + 1 ?>" class="form-label">Quantity</label>
                            <input type="number" class="form-control" id="quantity<?= $index + 1 ?>" name="quantities[]" value="<?= $ticketType['Quantity'] ?>" required>
                        </div>
                        <?php endforeach; ?>
                    </div>
                    <button type="button" class="btn btn-primary" onclick="addTicketType()">Add Ticket Type</button>
                    <hr>
                    <button type="submit" class="btn btn-success">Update Event with Tickets</button>
                </form>
            </div>
        </div>
